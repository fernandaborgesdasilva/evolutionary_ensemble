...
class BruteForceEnsembleClassifier:
    ...
    def fit(self, X, y, all_possible_ensembles):
        result_dict = dict()
        selected_ensemble = np.zeros([len(all_possible_ensembles)])
        random.seed(self.random_state)
        ...
        i = 0
        while i < (len(all_possible_ensembles)):
            ...
            classifier_id = 0
            if i <= self.stop_time:
                classifiers = random.choice(all_possible_ensembles)
                if selected_ensemble[classifiers[0]] == 0:
                    for cl in range(0, self.n_estimators):
                        classifier = classifiers[1][cl][0]
                        params = classifiers[1][cl][1]
