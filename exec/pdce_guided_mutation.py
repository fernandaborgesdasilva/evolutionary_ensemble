...
class Chromossome:
    ...
    def mutate(self, hyperparameter_proba, mutation_guided_before=0, n_positions=None):
...
class DiversityEnsembleClassifier:
    def __init__(self, algorithms, population_size = 100, max_epochs = 100, random_state=None):
        ...
        for i in range(0, population_size):
            self.population.append(Chromossome(genotypes_pool=self.algorithms, rnd=self.rnd, random_state=self.random_state))
        for i in range(0, population_size):
            self.population[i].mutate(self.hyperparameter_proba)
    ...
    def generate_offspring(self, parents, children, pop_fitness, mutation_guided_before):
        ...
        for i in range(0, self.population_size):
            ...
            new_chromossome.mutate(self.hyperparameter_proba, mutation_guided_before)
            try:
        ...
        for epoch in range(self.max_epochs):
            ...
            if (len(selected) < self.population_size):
                ...
                for i in range(0, diff):
                    ...
                    extra_classifier.mutate(self.hyperparameter_proba)                  
...
def compare_results(data, target, n_estimators, outputfile, stop_time, n_cores):
    ...
    with open(outputfile, "w") as text_file:
        text_file.write('*'*60)
        text_file.write(' Parallel DCE with guided mutation')
        text_file.write('*'*60)
        ...
        for train, val in kf.split(data):
            ...
            for i in range(0, 10):
                ...
                csv_file = 'pdce_guided_fold_' + str(fold) + '_iter_' + str(i) + '_' + str(n_cores) + '_' + time.strftime("%H_%M_%S", time.localtime(time.time())) + '.csv'
                print('\n\nIteration = ',i)
                text_file.write("\n\nIteration = %i" % (i))
...
def main(argv):
    ...
    try:
        ...
        print('pdce_guided_mutation.py -i <inputfile> -o <outputfile> -e <n_estimators> -s <stop_time> -c <n_cores>')
        sys.exit(2)
    if opts == []:
        print('pdce_guided_mutation.py -i <inputfile> -o <outputfile> -e <n_estimators> -s <stop_time> -c <n_cores>')
        sys.exit(2)
    ...
    if inputfile == "iris":
        ...
        print('Runing DCE with guided mutation (parallel version)...')
