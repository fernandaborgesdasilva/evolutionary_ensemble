...
class DiversityEnsembleClassifier:
    def __init__(self, algorithms, x_n_cols, population_size = 100, max_epochs = 100, random_state=None):
        ...
        for i in range(0, population_size):
            self.population.append(Chromossome(genotypes_pool=algorithms, x_n_cols=self.x_n_cols, rnd=self.rnd, random_state=self.random_state))
...
def compare_results(data, target, n_estimators, outputfile, stop_time, n_cores):
    ...
    with open(outputfile, "w") as text_file:
        text_file.write('*'*60)
        text_file.write(' PDCE (cols as gene version) ')
        text_file.write('*'*60)
        ...
        for train, val in kf.split(data):
            ...
            for i in range(0, 10):
                ...
                csv_file = 'pdce_cols_gene_fold_' + str(fold) + '_iter_' + str(i) + '_' + str(n_cores) + '_' + time.strftime("%H_%M_%S", time.localtime(time.time())) + '.csv'
                print('\n\nIteration = ',i)
                text_file.write("\n\nIteration = %i" % (i))
                ...
                text_file.write("\n\nPDEC fit done in %i" % (fit_total_time))
                text_file.write(" ms")
                ...
                text_file.write("\n\nPDEC predict done in %i" % (predict_total_time))
                text_file.write(" ms")
...
def main(argv):
    ...
    try:
        ...
        print('pdce_col_as_gene.py -i <inputfile> -o <outputfile> -e <n_estimators> -s <stop_time> -c <n_cores>')
        sys.exit(2)
    if opts == []:
        print('pdce_col_as_gene.py -i <inputfile> -o <outputfile> -e <n_estimators> -s <stop_time> -c <n_cores>')
        sys.exit(2)
    ...
    if inputfile == "iris":
        ...
        print('Runing PDCE (cols as gene version)...')
