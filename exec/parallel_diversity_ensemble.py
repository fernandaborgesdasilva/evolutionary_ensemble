...
class DiversityEnsembleClassifier:
    ...
    def fit(self, X, y):
        ...
        all_predictions = np.empty([2*self.population_size, y.shape[0]])
        ...
        for epoch in range(self.max_epochs):
            ...
            backend = 'loky'
            fit_predictions = Parallel(n_jobs=num_cores, backend=backend)(delayed(self.fit_predict_population)(item, kf, X, y) for item in not_selected)
            
            for i in fit_predictions:
                all_predictions[i[0]] = i[1]

            #print('Applying diversity selection...', end='')
            selected, diversity, fitness = self.diversity_selection(all_predictions, selection_threshold)
...
def main(argv):
    ...
    try:
        ...
        print('parallel_diversity_ensemble.py -i <inputfile> -o <outputfile> -e <n_estimators> -s <stop_time>')
        sys.exit(2)
    if opts == []:
        print('parallel_diversity_ensemble.py -i <inputfile> -o <outputfile> -e <n_estimators> -s <stop_time>')
        sys.exit(2)
