...
def compare_results(data, target, n_estimators, outputfile, stop_time, all_possible_ensembles):
    ...
    with open(outputfile, "w") as text_file:
        ...
        for i in range(0, 10):
            csv_file = 'parallel_brute_force_random_search_results_iter_' + str(i) + '_' + time.strftime("%H_%M_%S", time.localtime(time.time())) + '.csv'
...
def main(argv):
    ...
    try:
        ...
        print('parallel_brute_force_random_search_exec.py -i <inputfile> -o <outputfile> -e <n_estimators> -s <stop_time>')
        sys.exit(2)
    if opts == []:
        print('parallel_brute_force_random_search_exec.py -i <inputfile> -o <outputfile> -e <n_estimators> -s <stop_time>')
        sys.exit(2)
